...
class UE:
    ...
    POS = {0: Pos(4.5, 2.5),  # list of users, including their location
           1: Pos(11.5, 6.5),
           2: Pos(10.5, 1.5)}  # which is on the grid
...
def get_reward(state):
    ...
    for ue_id in UE.RATE:
        all_rates.append(UE.RATE[ue_id][state.row][state.col])
...
def main_loop(ai):
    ...
    while running:
        ...
        if STAT.step == UAV.FLIGHT_TIME or sim.uav_pos == UAV.END_POS:
            ...
            # Return too early? Apply penalty
            ...
            if repeat_round["flag"]:
                ...
                if STAT.round % SAMPLE_INTERVAL == 0:
                    ...
                    STAT.round -= 1  # Step one round back to repeat the same round
...
# Try this one
ai = PPO(exploration=EXPLORATION)
#ai = viv_DeepQLearning(exploration=EXPLORATION)
# ai = DeepQLearning(exploration=EXPLORATION)

print(f"Running {ai.name} algorithm...")

if not EXPLORATION:
    print("- NO exploration mode is in place")
