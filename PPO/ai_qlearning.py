...
class Q_Learning(RL):
    ...
    def q(self, state):
        ...
        if state not in self.q_table:
            ...
            self.q_table[state] = np.zeros(Action.COUNT)
        return self.q_table[state]

    def execute(self, state, reward) -> int:
        # print(f"Q-Learning: {state} -> {reward}")
        '''It returns an action based on Q-learning.'''
        ...
        ## update Q-table using using Bellman equation 
        ## based on the received reward:
        ## Q_next(s,a) = Q(s,a) \
        ##               + alpha * (reward + gamma*max_a(Q(s_next,a)) - Q(s,a))
        ##             = (1-alpha) * Q(s,a)
        ##               + alpha  * (reward + gamma*max_a(Q(s_next,a)))
        if self.current_state is not None: # first time? then skip update
